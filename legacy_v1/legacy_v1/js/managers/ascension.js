...
const Ascension = {
  ...
  // v30.4 PERK TOGGLE
  togglePerk(perkId) {
      if (!this.upgrades[perkId]) return; // Not owned
      
      const idx = this.equippedPerks.indexOf(perkId);
      if (idx > -1) {
          this.equippedPerks.splice(idx, 1);
          UI.toast("Perk Unequipped");
      } else {
          // Check limit
          const limit = Achievements ? Achievements.getPerkSlots() : 3;
          if (this.equippedPerks.length >= limit) {
              UI.toast(`Max Perks (${limit}) Reached!`);
              return;
          }
          this.equippedPerks.push(perkId);
          UI.toast("Perk Equipped");
      }
      this.save();
      // If UI is open, refresh it? Vue reactivity handles it if store is updated.
      // But Ascension.js is external.
      // We rely on Vue computed props reading Ascension object.
      // Ideally we trigger a reactive update or re-render.
